...
import {
  ProgressDomainException,
  ProgressNotFoundException,
} from "src/domain/exceptions/progress.exceptions";
...
export class SubmitQuizAttemptUseCase implements ISubmitQuizAttemptUseCase {
  ...
  async execute(input: {
    ...
    const quiz = await this._quizRepo.findById(input.quizId);
    if (!quiz) {
    ...
    const progressEntry = await this._progressRepo.findByEnrollmentIdAndQuizId(
      input.enrollmentId,
      input.quizId,
    );
    if (!progressEntry) {
