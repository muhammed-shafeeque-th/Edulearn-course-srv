import { ModuleDomainException } from "../exceptions/module.exceptions";
...
export class Module {
  ...
  constructor(props: ModuleProps) {
    if (!props.id?.trim())
      throw new ModuleDomainException("Module ID is required.");
    if (!props.courseId?.trim())
      throw new ModuleDomainException("Course ID is required.");
    if (!props.title?.trim())
      throw new ModuleDomainException("Module title is required.");
  ...
  updateDetails(
    details: Partial<
      Pick<ModuleProps, "title" | "order" | "description" | "isPublished">
    >,
  ): void {
  ...
  addQuiz(quiz: Quiz): void {
    if (!quiz) throw new ModuleDomainException("Quiz is required.");
    this.quiz = quiz;
  ...
  addLesson(lesson: Lesson): void {
    if (!lesson) throw new ModuleDomainException("Lesson is required.");
    if (this.lessons.some((l) => l.getId() === lesson.getId())) {
      throw new ModuleDomainException(
        "Lesson with this ID already exists in this module.",
      );
    }
  ...
  removeLesson(lessonId: string): void {
    ...
    if (idx === -1) throw new ModuleDomainException("Lesson not found.");
  ...
  updateLesson(updatedLesson: Lesson): void {
    ...
    if (idx === -1)
      throw new ModuleDomainException("Lesson not found in this module.");
