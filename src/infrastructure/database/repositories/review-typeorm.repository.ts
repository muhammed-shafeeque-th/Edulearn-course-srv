...
import {
  IReviewRepository,
  ReviewBreakDown,
} from "src/domain/repositories/review.repository";
...
export class ReviewTypeOrmRepository
  extends BaseRepository<Review, ReviewOrmEntity>
  implements IReviewRepository
{
  protected contextName: string = ReviewTypeOrmRepository.name;
  constructor(
    @InjectRepository(ReviewOrmEntity)
    repo: Repository<ReviewOrmEntity>,
  ...
  async findByUserAndCourse(
    ...
    return await this.execute("findByUserAndCourse", async (span) => {
      span.setAttributes({
        "db.review.operation": "SELECT",
        "review.course.id": courseId,
  ...
  async findByEnrollmentId(enrollmentId: string): Promise<Review | null> {
    return await this.execute("findByEnrollmentId", async (span) => {
      span.setAttributes({
        "db.review.operation": "SELECT",
  ...
  async getCourseRatingsBreakdown(courseId: string): Promise<ReviewBreakDown> {
    return await this.execute("getCourseRatingsBreakdown", async (span) => {
