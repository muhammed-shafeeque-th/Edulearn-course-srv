...
@Injectable()
export class ProgressTypeOrmRepository
  extends BaseRepository<Progress, ProgressOrmEntity>
  implements IProgressRepository
{
  protected contextName: string = ProgressTypeOrmRepository.name;
  constructor(
    @InjectRepository(ProgressOrmEntity)
    repo: Repository<ProgressOrmEntity>,
  ...
  async save(progress: Progress): Promise<void> {
    return this.execute("save", async (span) => {
      try {
        ...
        //   ),
        // ]);
        // const invalidated = CACHE_KEYS.progress.byInvalidateOnProgressChange(
        //   progress.getId(),
  ...
  async findByEnrollmentIdAndQuizId(
    ...
    return this.execute("findByEnrollmentIdAndQuizId", async (span) => {
      try {
        span.setAttributes({
          "db.operation": "SELECT",
        ...
        // const cacheKey = CACHE_KEYS.progress.byEnrollmentQuiz(
        //   enrollmentId,
        //   quizId,
        // );
  ...
  async findById(id: string): Promise<Progress | null> {
    return this.execute("findById", async (span) => {
      try {
        ...
        this.logger.error(`Failed to find progress by id: ${error.message}`, {
          ctx: ProgressTypeOrmRepository.name,
        });
        span.setAttribute("error", true);
        throw error;
  ...
  async findByEnrollmentId(enrollmentId: string): Promise<Progress[]> {
    return this.execute("findByEnrollmentId", async (span) => {
      try {
        ...
        const progressEntries = ormEntities.map(
  ...
  async findByEnrollmentIdAndLessonId(
    ...
    return this.execute("findByEnrollmentIdAndLessonId", async (span) => {
      try {
        span.setAttributes({
          "db.operation": "SELECT",
          "enrollment.id": enrollmentId,
