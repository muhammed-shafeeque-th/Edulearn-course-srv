...
export const CACHE_KEYS = {
  course: {
    byId: (id: string, options?: CourseRelationOptions) =>
      `course:${id}:modules:${options?.withModules ?? null}:lessons:${options?.withLessons ?? null}:quizzes:${options?.withQuiz ?? null}`,
    ...
    list: (
      params: Omit<GetCourseParams, "category" | "level"> & {
        category: string[];
        level: string[];
      },
    ) => {
    ...
    instructorWildcard: (instructorId: string) =>
      `courses:instructor:${instructorId}:*`,
  },
  module: {
    ...
    byCourse(courseId: string): string {
      return `modules:course:${courseId}`;
    },

    invalidateKeys(moduleId: string, courseId: string): string[] {
      return [
        CACHE_KEYS.module.byId(moduleId),
        CACHE_KEYS.module.byCourse(courseId),
      ];
    },
  },
  lesson: {
  ...
  quiz: {
    ...
    invalidateKeys(
      ...
      return [
        ...
        CACHE_KEYS.quiz.byModule(moduleId),
  ...
  review: {
    ...
    courseRatingsBreakdown: (courseId: string) =>
      `review:course:${courseId}:ratings_breakdown`,
    byCourseWithParams: (
      courseId: string,
      page: number,
      limit: number,
  ...
  certificate: {
    ...
    byEnrollmentId: (enrollmentId: string) =>
      `certificate:enrollment:${enrollmentId}`,
    byCertificateNumber: (certificateNumber: string) =>
  ...
  enrollment: {
    ...
    courseIdsByStudent: (studentId: string) =>
      `enrollments:user:${studentId}:course-ids`,
    studentIdsByCourse: (courseId: string) =>
      `enrollments:course:${courseId}:student-ids`,
  },
  category: {
    all: "categories:all",
